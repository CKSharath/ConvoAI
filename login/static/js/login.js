...
const canvas = document.getElementById('face-canvas'); // Added canvas element reference
...
    // const canvas = document.getElementById('face-canvas'); // Already declared globally
...
        // --- START FIX: Correctly structure the form data and request ---
        const formData = new FormData();
        
        // 1. Append the file data BEFORE sending the request
        formData.append('file', blob, 'face_capture.jpeg');

        try {
            // 2. Send a single request with the file in the body and user_id in the URL
            const response = await fetch(`/mfa/face_verify?user_id=${currentUserId}`, {
                method: 'POST',
                // No headers needed for FormData, browser sets it automatically
                body: formData 
            });
...
            if (!response.ok) {
                // This will now correctly display the 403 or 422 detail message
                showMessage(data.detail || 'Face verification failed.', true); 
                return;
            }

            // --- END FIX: Logic continues below ---
